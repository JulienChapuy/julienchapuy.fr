...
const Navbar: React.FC<NavbarProps> = ({ initialLang, initialPath, allLabels, allContact }) => {
    ...
    const [lang, setLang] = React.useState<'fr' | 'en'>(getLangFromPath(initialPath));
    ...
    const isActive = (path: string) => {
        if (path === '/' && (currentPath === '/' || currentPath === '/en' || currentPath === '/en/')) return true;
    ...
    const toggleLangLink = lang === 'en'
        ? (currentPath.replace('/en', '') || '/')
        : (`/en${currentPath === '/' ? '' : currentPath}`);

    return (
        <>
            <nav className={`${styles.navbar} ${isScrolled ? styles.scrolled : ''}`} id="mainNav">
                <div className={styles['nav-container']}>
                    ...
                    <div className={styles['nav-center']}>
                        <ul className={styles['nav-links']}>
                            <li>
                                <a className={`${isActive('/') ? styles.active : ''}`} href={homeLink}>{labels.home}</a>
                            </li>
                            <li>
                                <a className={`${isActive('about') ? styles.active : ''}`} href={aboutLink}>{labels.about}</a>
                            </li>
                            <li>
                                <a className={`${isActive('projects') ? styles.active : ''}`} href={projectsLink}>{labels.work}</a>
                            </li>
                            <li>
                                <a className={`${isActive('blog') ? styles.active : ''}`} href={blogLink}>{labels.blog}</a>
                            </li>
                            <li>
                                <a className={`${isActive('resume') ? styles.active : ''}`} href={resumeLink}>{labels.resume}</a>
                            </li>
                        </ul>
                    </div>

                    <div className={styles['nav-right']}>
                        <button className={styles['btn-contact']} onClick={() => setIsModalOpen(true)}>
                            {labels.contact}
                        </button>
                        <a className={styles['lang-toggle']} href={toggleLangLink}>{lang === 'fr' ? 'EN' : 'FR'}</a>
                        <ThemeToggle />
                    </div>
                </div>
            </nav>
